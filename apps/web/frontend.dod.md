...
- **UI Library:** Mantine UI, Tailwind CSS (if applicable)
...
- [ ] **UI Consistency:** Use only Mantine UI theme tokens for spacing, colors, and typography. No hardcoded hex colors or arbitrary pixel values in components.
...
- [ ] **Data Fetching:** Direct API calls inside `useEffect` are PROHIBITED. All server state interactions must be handled via TanStack Query (`useQuery`, `useMutation`) or Next.js Server Actions.
...
- [ ] **Error Handling:** Unhandled promise rejections must be caught. API errors must display user-friendly Toast notifications, not raw error objects or stack traces.
...
- [ ] **State Security:** Sensitive information (JWT tokens, PII) MUST NOT be stored in `localStorage`. Rely on secure HttpOnly cookies managed via BFF or Next.js API routes.
